print("Test branch loaded. (Overlay + Smooth UI)")
...
-- Theme
local theme = {
    bg = Color3.fromRGB(25, 25, 30),         -- Ciemnoszary
    header = Color3.fromRGB(35, 35, 40),     -- Nieco jasniejszy naglowek
    outline = Color3.fromRGB(60, 60, 65),    -- Subtelny obrys
    separator = Color3.fromRGB(100, 140, 200), -- Stonowany niebieski akcent
    
    text = Color3.fromRGB(225, 225, 230),    -- Zlamana biel
    ...
    q_active = Color3.fromRGB(80, 160, 120), -- Lagodna zielen
...
local isOverlayMode = false -- Stan Overlay
...
TitleText.Font=2; -- Monospace
TitleText.Outline=false; -- Bez cienia
table.insert(ui, TitleText)

-- Top Buttons (Clean Font)
local BtnGallery = Drawing.new("Text"); BtnGallery.Text="[ GALLERY ]"; BtnGallery.Size=13; BtnGallery.Color=theme.accent; BtnGallery.Visible=true; BtnGallery.Font=2; BtnGallery.Outline=false; table.insert(ui, BtnGallery)
local BtnSettings = Drawing.new("Text"); BtnSettings.Text="[ SETTINGS ]"; BtnSettings.Size=13; BtnSettings.Color=theme.text_dim; BtnSettings.Visible=true; BtnSettings.Font=2; BtnSettings.Outline=false; table.insert(ui, BtnSettings)

-- Indicators (Clean Font)
local QualInd = Drawing.new("Text"); QualInd.Text=currentMode; QualInd.Size=13; QualInd.Color=theme.text_dim; QualInd.Transparency=0.6; QualInd.Visible=true; QualInd.Font=2; QualInd.Outline=false; table.insert(ui, QualInd)
...
local HiddenLabel = Drawing.new("Text"); HiddenLabel.Text="Menu hidden ("..keyName..")"; HiddenLabel.Size=16; HiddenLabel.Color=Color3.fromRGB(255,255,255); HiddenLabel.Transparency=0.5; HiddenLabel.Visible=false; HiddenLabel.Center=true; HiddenLabel.Font=2; HiddenLabel.Outline=true
...
local function updateVisibility()
    ...
    elseif isOverlayMode then
        -- Overlay: Ukryj UI, zostaw piksele
        for _, v in pairs(ui) do v.Visible = false end
        clearPanel()
...
local function createMenuBtn(text, color, y, type, val, w)
    ...
    btnText.Font = 2; btnText.Outline = false -- Clean Font
...
local function buildGallery()
    ...
    local yOff = 10
    local head = Drawing.new("Text"); head.Text="GALLERY"; head.Size=14; head.Color=theme.text_dim; head.Visible=isVisible; head.ZIndex=22; head.Font=2; head.Outline=false
...
local function buildSettings()
    clearPanel(); PanelBg.Visible = isVisible; PanelOutline.Visible = isVisible
    local yOff = 10
    local head = Drawing.new("Text"); head.Text="SETTINGS"; head.Size=14; head.Color=theme.text_dim; head.Visible=isVisible; head.ZIndex=22; head.Font=2; head.Outline=false
    table.insert(btns, {obj=head, type="deco", relX=10, relY=yOff}); yOff=yOff+25

    local kLbl = Drawing.new("Text"); kLbl.Text="Toggle:"; kLbl.Size=13; kLbl.Color=theme.text; kLbl.Visible=isVisible; kLbl.ZIndex=22; kLbl.Font=2; kLbl.Outline=false
    table.insert(btns, {obj=kLbl, type="deco", relX=10, relY=yOff}); 
    local kVal = Drawing.new("Text"); kVal.Text="[" .. keyName .. "]"; kVal.Size=13; kVal.Color=theme.accent; kVal.Visible=isVisible; kVal.ZIndex=22; kVal.Font=2; kVal.Outline=false
    table.insert(btns, {obj=kVal, type="bind_menu", relX=90, relY=yOff}); yOff=yOff+20

    local oLbl = Drawing.new("Text"); oLbl.Text="Overlay:"; oLbl.Size=13; oLbl.Color=theme.text; oLbl.Visible=isVisible; oLbl.ZIndex=22; oLbl.Font=2; oLbl.Outline=false
    table.insert(btns, {obj=oLbl, type="deco", relX=10, relY=yOff}); 
    local oVal = Drawing.new("Text"); oVal.Text="[" .. overlayKeyName .. "]"; oVal.Size=13; oVal.Color=theme.accent; oVal.Visible=isVisible; oVal.ZIndex=22; oVal.Font=2; oVal.Outline=false
    table.insert(btns, {obj=oVal, type="bind_overlay", relX=90, relY=yOff}); yOff=yOff+30

    local qLbl = Drawing.new("Text"); qLbl.Text="Quality:"; qLbl.Size=13; qLbl.Color=theme.text; qLbl.Visible=isVisible; qLbl.ZIndex=22; qLbl.Font=2; qLbl.Outline=false
    ...
    for i, mode in ipairs(modes) do
        ...
        local bg = Drawing.new("Square"); bg.Size = Vector2.new(btnW, 18); bg.Color = isActive and theme.q_active or theme.q_idle; bg.Filled = true; bg.Visible = isVisible; bg.ZIndex = 21
        table.insert(btns, {obj=bg, type="q_"..string.lower(mode), relX=startX, relY=yOff})
        local txt = Drawing.new("Text"); txt.Text = mode; txt.Size = 11; txt.Color = isActive and Color3.new(0,0,0) or Color3.new(1,1,1); txt.Visible = isVisible; txt.ZIndex = 22; txt.Font=2; txt.Outline=false; txt.Center=true
        table.insert(btns, {obj=txt, type="deco", relX=startX+(btnW/2), relY=yOff+2})
        startX = startX + btnW + 5
    end
    yOff = yOff + 30

    local opLbl = Drawing.new("Text"); opLbl.Text="Opacity: " .. math.floor(currentOpacity*100) .. "%"; opLbl.Size=13; opLbl.Color=theme.text; opLbl.Visible=isVisible; opLbl.ZIndex=22; opLbl.Font=2; opLbl.Outline=false
    table.insert(btns, {obj=opLbl, type="op_lbl", relX=10, relY=yOff}); yOff=yOff+20
    local barBg = Drawing.new("Square"); barBg.Size=Vector2.new(130, 6); barBg.Color=Color3.fromRGB(50,50,50); barBg.Filled=true; barBg.Visible=isVisible; barBg.ZIndex=21
    table.insert(btns, {obj=barBg, type="op_bar", relX=10, relY=yOff})
    local barFill = Drawing.new("Square"); barFill.Size=Vector2.new(130 * currentOpacity, 6); barFill.Color=theme.accent; barFill.Filled=true; barFill.Visible=isVisible; barFill.ZIndex=22
