...
elseif DATABASE["Demo"] then
    curData = DATABASE["Demo"][1]
else
    local k, v = next(DATABASE)
    if v then curData = v[1] end
end
...
local function createMenuBtn(text, color, y, type, val, w)
    ...
    btnText.Font = 2; btnText.Outline = false
...
local function buildSettings()
    ...
    for i, mode in ipairs(modes) do
        ...
        table.insert(btns, {obj=txt, type="deco", relX=startX+(btnW/2), relY=yOff+1}) 
...
task.spawn(function()
    while true do
        ...
        local isActiveWindow = true
        if type(iswindowactive) == "function" then
            isActiveWindow = iswindowactive()
        end
        ...
        if isActiveWindow then
            ...
            if not rebindingAction then
                ...
                for k = 8, 255 do
                    if iskeypressed(k) and k ~= 0 then
                        ...
                        rebindingAction = nil
                        buildSettings(); updatePositions(windowPosX, windowPosY); task.wait(0.2); break
                    end
                end
            end
            ...
            if isVisible and not isOverlayMode then
                ...
                if draggingSlider and m1Now then
                    for _, b in ipairs(btns) do
                        if b.type == "op_bar" then
                            local relX = mx - b.obj.Position.X
                            currentOpacity = math.clamp(relX / 130, 0.1, 1.0)
                            renderPixels(); buildSettings(); updatePositions(windowPosX, windowPosY)
                        end
                    end
                else
